...
class Model:
    ...
    def load(self, restore_iter):
        full_checkpoint_path = osp.join(os.getcwd(),'work', 'checkpoint', self.model_name,
            '{}-{:0>5}-encoder.ptm'.format(self.save_name, restore_iter))
        full_optimizer_path = osp.join(os.getcwd(), 'work','checkpoint', self.model_name,
            '{}-{:0>5}-optimizer.ptm'.format(self.save_name, restore_iter))

        print(f"DEBUG PATH: Trying to load from: {full_checkpoint_path}") # Pentru verificare
        self.encoder.load_state_dict(torch.load(full_checkpoint_path))
        self.optimizer.load_state_dict(torch.load(full_optimizer_path))