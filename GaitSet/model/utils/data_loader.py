...
def load_data(dataset_path, resolution, dataset, pid_num, pid_shuffle, cache=True):
    ...
    if not osp.exists(pid_fname):
        ...
        # Fix: Convert to object array and allow pickling to handle the list of lists with unequal lengths
        np.save(pid_fname, np.array(pid_list, dtype=object), allow_pickle=True)
